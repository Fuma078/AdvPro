...
public class GameStage extends Pane {
    ...
    public void render(Snake snake, Food food) {
        ...
        // Render snake in blue
        gc.setFill(Color.BLUE);
        snake.getBody().forEach(p -> {
            gc.fillRect(p.getX() * TILE_SIZE, p.getY() * TILE_SIZE, TILE_SIZE, TILE_SIZE);
        });

        // Render food with different colors based on type
        if (food.getFoodType() == Food.FoodType.SPECIAL) {
            gc.setFill(Color.GREEN); // Special food is green
        } else {
            gc.setFill(Color.RED); // Standard food is red
        }
